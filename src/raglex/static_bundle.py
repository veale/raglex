...
def render_index_html(
    ...
    """``entries`` are the built editions: filename, title, exported date, counts, note."""
    ...
    for entry in entries:
        meta = [f"exported {entry.get('exported', format_export_date(generated_at))}"]
        if entry.get("documents"):
            meta.append(f"{entry['documents']:,} citing documents")
        note_html = sanitise_editorial_html(
            apply_placeholders(entry.get("note") or "", when=generated_at,
                               count=len(entries)))
        rows.append(
            "        <li>\n"
            f'          <a href="{escape(entry["filename"], quote=True)}">'
...
def build_bundle(
    ...
    result = {
        ...
        "files": [
            {k: entry[k] for k in ("filename", "title", "documents", "bytes", "exported")}
            for entry in entries
        ],
