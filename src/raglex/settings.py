...
KNOWN_SETTINGS: tuple[SettingSpec, ...] = (
    ...
    SettingSpec("RAGLEX_EMBED_JURISDICTIONS", "Embed only these jurisdictions", False, "Embeddings",
                "comma-separated jurisdiction names (e.g. United Kingdom, European Union); "
                "blank = embed the whole corpus. Indexing all ~5M docs is infeasible on a "
                "small box — scope it to what you actually search"),
    ...
    # HPC (UCL Myriad) bulk-embed relay (hpc/README.md; `raglex hpc-embed`).
    SettingSpec("RAGLEX_HPC_HOST", "HPC SSH host alias", False, "HPC embed",
                "an entry in ~/.ssh/config (ProxyJump/keys live there); default 'myriad'"),
    SettingSpec("RAGLEX_HPC_REMOTE_DIR", "HPC scratch dir", False, "HPC embed",
                "default ~/Scratch/raglex-embed"),
    SettingSpec("RAGLEX_HPC_MODEL", "HPC embed model", False, "HPC embed",
                "HF model id (blank = RAGLEX_EMBED_MODEL or Qwen/Qwen3-Embedding-0.6B)"),
    SettingSpec("RAGLEX_HPC_DIMENSIONS", "HPC vector width", False, "HPC embed",
                "e.g. 1024 (512 halves disk via Matryoshka truncation — Qwen3 only)"),
    SettingSpec("RAGLEX_HPC_NTASKS", "HPC array tasks", False, "HPC embed",
                "GPU array size; must match the jobscript -t range (default 40)"),
    SettingSpec("RAGLEX_HPC_WALLCLOCK", "HPC per-task wallclock", False, "HPC embed",
                "SGE h_rt per task, e.g. 8:00:00"),
    SettingSpec("RAGLEX_HPC_DEADLINE_HOURS", "HPC overall deadline (h)", False, "HPC embed",
                "orchestrator stops (resumably) past this — guards against runaway GPU spend; default 24"),
    SettingSpec("RAGLEX_HPC_POLL_SECONDS", "HPC poll interval (s)", False, "HPC embed",
                "qstat cadence while queued/running; default 120"),
