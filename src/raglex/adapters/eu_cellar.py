...
# -- pure Formex helpers ----------------------------------------------------
#: An OJ Formex archive holds a CHAIN of two wrappers before the text, and both of them
#: look like documents:
#:
#:   C_202402318EN.toc.fmx.xml       <PUBLICATION>  the issue's masthead / contents
#:   C_202402318EN.doc.fmx.xml       <DOC>          a manifest — bibliographic fields
#:                                                  and <REF.PHYS FILE="…000101…"/>
#:   C_202402318EN.000101.fmx.xml    <CJT>          the notice itself
#:
#: Taking the archive's first member stored the masthead as 993 notices' text. Naming
#: the ".doc." member instead — on the reasoning that it is "the document instance" —
#: swapped one wrapper for the other: the manifest parses to its own field values
#: ("20240315034 483657 2024 2318 T ELI:…"), which is why the repair pass produced
#: "Pending: Case T-48/24" a second time. The member is identified by its ROOT ELEMENT,
#: which says what a file IS; the filename only says where it sits in the issue.
_PUBLICATION_ROOT = re.compile(rb"<\s*PUBLICATION[\s>]", re.IGNORECASE)
#: The manifest root. Distinguished from a content root by <BIB.DOC>, which only the
#: manifest carries — a content instance carries <BIB.INSTANCE>.
_DOC_MANIFEST_ROOT = re.compile(rb"<\s*DOC[\s>].{0,400}?<\s*BIB\.DOC[\s>]",
                                re.IGNORECASE | re.DOTALL)
#: Each wrapper names the next link down: the masthead by DOC.INSTANCE, the manifest by
#: REF.PHYS FILE. Following them beats guessing at filenames.
_DOC_INSTANCE = re.compile(rb'DOC\.INSTANCE\s*=\s*"([^"]+)"', re.IGNORECASE)
_REF_PHYS = re.compile(rb'<\s*REF\.PHYS[^>]*\bFILE\s*=\s*"([^"]+)"', re.IGNORECASE)


def _is_wrapper(blob: bytes) -> bool:
    """Whether a Formex member is apparatus (masthead or manifest) rather than text."""
    head = blob[:1200]
    return bool(_PUBLICATION_ROOT.search(head) or _DOC_MANIFEST_ROOT.search(head))
...
def unzip_formex(raw: bytes) -> bytes | None:
    ...
    Which member is the document is decided by its ROOT ELEMENT, not its name: an OJ
    issue ships a masthead AND a bibliographic manifest alongside the item, and both are
    named as plausibly as the text is (see ``_DOC_MANIFEST_ROOT``). Follow each
    wrapper's own pointer down, and verify the destination is not itself apparatus.
    """
    if raw[:2] == b"PK":
        try:
            with zipfile.ZipFile(io.BytesIO(raw)) as zf:
                names = [n for n in zf.namelist()
                         if n.lower().endswith((".xml", ".fmx", ".fmx4"))]
                if not names:
                    names = zf.namelist()
                if not names:
                    return None
                members = {n.rsplit("/", 1)[-1]: n for n in names}

                def _follow(blob: bytes, seen: set[str]) -> bytes | None:
                    """Chase REF.PHYS / DOC.INSTANCE down to the first non-wrapper."""
                    for pattern in (_REF_PHYS, _DOC_INSTANCE):
                        for hit in pattern.finditer(blob):
                            wanted = hit.group(1).decode(errors="ignore").rsplit("/", 1)[-1]
                            name = members.get(wanted)
                            if not name or name in seen:
                                continue
                            seen.add(name)
                            nxt = zf.read(name)
                            if not _is_wrapper(nxt):
                                return nxt
                            deeper = _follow(nxt, seen)
                            if deeper is not None:
                                return deeper
                    return None

                # The ordinary archive holds exactly one member that is not apparatus.
                # Reading the roots settles it without trusting a naming convention.
                content = [n for n in names if not _is_wrapper(zf.read(n))]
                if len(content) == 1:
                    return zf.read(content[0])
                first = zf.read(names[0])
                if content:
                    # Several content members: follow the manifest, so the one chosen is
                    # the publication's main document rather than the zip's first.
                    followed = _follow(first, {names[0]}) if _is_wrapper(first) else None
                    return followed if followed is not None else zf.read(content[0])
                # Everything looks like apparatus — follow the pointers anyway, then
                # fall back to what earlier versions returned.
                followed = _follow(first, {names[0]})
                return followed if followed is not None else first
